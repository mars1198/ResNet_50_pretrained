...
def create_graph():
    ...
    with tf.io.gfile.GFile(os.path.join(inceptionV3_dir, 'resnet50_v1.pb'), 'rb') as f:
...
def extract_features(list_images):
    ...
    with tf.compat.v1.Session() as sess:
        next_to_last_tensor = sess.graph.get_tensor_by_name('resnet_model/final_reduce_mean:0')
        ...
        for ind, image in enumerate(list_images):
            ...
            predictions = sess.run(next_to_last_tensor, {'input_tensor:0': decoded_image_float})
...
#resnet_model/final_reduce_mean
